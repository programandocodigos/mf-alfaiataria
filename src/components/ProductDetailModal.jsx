...
export default function ProductDetailModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-5xl bg-aurum-surface rounded-xl border border-aurum-gold/30 shadow-card-dark overflow-hidden z-10 flex flex-col lg:flex-row my-8">
        
        {/* Close Button */}
        <button
          onClick={() => setSelectedProductDetail(null)}
          className="absolute top-4 right-4 z-20 p-2 rounded-full bg-aurum-bg/80 hover:bg-aurum-gold text-gray-300 hover:text-black transition-colors border border-aurum-gold/20"
          aria-label="Fechar"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="lg:w-1/2 p-6 bg-aurum-card flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-aurum-border/60">
          {/* Main Large Image */}
          <div className="relative aspect-[3/4] w-full rounded-lg overflow-hidden bg-aurum-bg border border-aurum-border">
            <img 
            ...
            />
            {product.badge && (
              <span className="absolute top-4 left-4 bg-aurum-gold text-black text-xs uppercase font-bold tracking-widest px-3 py-1 rounded shadow-gold-sm">
                {product.badge}
              </span>
            )}
          </div>
        ...
        <div className="lg:w-1/2 p-6 sm:p-8 flex flex-col justify-between space-y-6">
          <div>
            ...
            <div className="flex items-center justify-between text-xs text-aurum-gold uppercase tracking-wider font-semibold mb-2">
              ...
              <div className="flex items-center gap-1.5 bg-aurum-card px-2.5 py-1 rounded border border-aurum-border">
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              <span className="text-xs text-aurum-gold-champagne bg-aurum-gold/10 px-2 py-0.5 rounded border border-aurum-gold/20">
            ...
            <div className="space-y-2.5 mb-6 text-xs">
              <div className="bg-aurum-card/80 p-3 rounded border border-aurum-border/60 flex items-start gap-2.5">
                <strong className="text-aurum-gold font-semibold uppercase tracking-wider min-w-[90px]">Tecido:</strong>
                <span className="text-gray-200">{product.fabric}</span>
              </div>

              <div className="bg-aurum-card/80 p-3 rounded border border-aurum-border/60 flex items-start gap-2.5">
                <strong className="text-aurum-gold font-semibold uppercase tracking-wider min-w-[90px]">Corte:</strong>
                <span className="text-gray-200">{product.fit}</span>
              </div>

              <div className="bg-aurum-card/80 p-3 rounded border border-aurum-border/60 flex items-start gap-2.5">
                <strong className="text-aurum-gold font-semibold uppercase tracking-wider min-w-[90px]">Ocasião:</strong>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-xs text-aurum-gold hover:underline flex items-center gap-1 font-medium"
                >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex items-center bg-aurum-card rounded border border-aurum-border">
                <button
          ...
          <div className="pt-4 border-t border-aurum-border/60 space-y-3">
            <button
              ...
              className="w-full py-4 rounded-md bg-gradient-to-r from-aurum-gold-dark via-aurum-gold to-aurum-gold-light text-black font-bold uppercase tracking-wider text-xs flex items-center justify-center gap-2 shadow-gold-glow hover:brightness-110 transition-all transform hover:-translate-y-0.5"
            >
          ...
          {/* Look Builder - smart product combinations */}
