...
export default function ProductDetailModal() {
  ...
  const productId = product?.id || product?.cartItemId;

  const [activeImageIndex, setActiveImageIndex] = useState(0);
  const [selectedSize, setSelectedSize] = useState('');
  const [quantity, setQuantity] = useState(1);

  // 1. Lock body background scroll when modal is open
  useEffect(() => {
  ...
  // 2. Reset scroll position to top & reset states when switching products (e.g. from Complete o Look)
  useEffect(() => {
    if (product) {
      setActiveImageIndex(0);
      const defaultSize = Array.isArray(product.sizes) && product.sizes.length > 0 ? product.sizes[0] : 'Tamanho Único';
      setSelectedSize(defaultSize);
      setQuantity(1);

      // Scroll modal container back to top smoothly
  ...
  const handleAddToCart = () => {
    addToCart(product, selectedSize || product.sizes?.[0] || 'Tamanho Único', quantity);
  ...
  return (
    <div 
      ref={overlayRef}
      className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-md p-4 sm:p-6 animate-fadeIn"
    >
      ...
      {/* Modal Alignment Wrapper: min-h-full items-start prevents negative overflow clipping */}
      <div className="min-h-full flex items-start justify-center py-4 sm:py-8">
        
        {/* Modal Card Container */}
        <div className="relative w-full max-w-5xl bg-aurum-surface rounded-2xl border border-aurum-gold/30 shadow-gold-glow overflow-hidden z-10 flex flex-col my-auto">
          
          {/* Close Button */}
          <button
            onClick={() => setSelectedProductDetail(null)}
            className="absolute top-4 right-4 z-30 p-2.5 rounded-full bg-aurum-bg/90 hover:bg-aurum-gold text-gray-300 hover:text-black transition-colors border border-aurum-gold/30 shadow-lg cursor-pointer"
            aria-label="Fechar"
          >
            <X className="w-5 h-5" />
          </button>
              ...
              <div>
                ...
                <div className="space-y-2 mb-6 text-xs">
                  <div className="bg-aurum-card/80 p-3 rounded-xl border border-aurum-border/60 flex items-start gap-2.5">
                    ...
                    <span className="text-gray-200">{product.fabric || 'Alta Alfaiataria'}</span>
                  </div>

                  <div className="bg-aurum-card/80 p-3 rounded-xl border border-aurum-border/60 flex items-start gap-2.5">
                    <strong className="text-aurum-gold font-semibold uppercase tracking-wider min-w-[85px]">Corte:</strong>
                    <span className="text-gray-200">{product.fit || 'Sartorial Sob Medida'}</span>
                  </div>

                  <div className="bg-aurum-card/80 p-3 rounded-xl border border-aurum-border/60 flex items-start gap-2.5">
                    <strong className="text-aurum-gold font-semibold uppercase tracking-wider min-w-[85px]">Ocasião:</strong>
                    <span className="text-gray-200">{product.occasion || 'Eventos Formais'}</span>
                  </div>
                </div>
          ...
          {/* Bottom Full-Width Section: Complete o Look */}
